...
- [ ] Unique user IDs are assigned - no shared or generic accounts (CWE-287)
- [ ] Role-based access control is enforced at the application layer (CWE-862)
- [ ] Least privilege principle applied - users only access what their role requires (CWE-285)
...
- [ ] User provisioning and de-provisioning workflows exist (CWE-262)
...
- [ ] Network segmentation isolates sensitive components from public-facing services (CWE-668)
...
- [ ] Audit logs are tamper-proof (append-only storage or integrity protection) (CWE-779)
...
- [ ] Log retention meets SOC 2 audit window requirements (minimum 12 months) (CWE-779)
...
- [ ] Code changes require pull request review before merge (CWE-829)
- [ ] CI/CD pipelines run automated tests before deployment (CWE-829)
- [ ] Production deployments require approval from authorized personnel (CWE-829)
- [ ] Rollback procedures are documented and tested (CWE-829)
- [ ] Infrastructure changes are managed via IaC (Terraform, CloudFormation) with version control (CWE-829)
- [ ] Dependency updates are reviewed for security implications (CWE-1104)
- [ ] Emergency change procedures exist with post-incident review (CWE-829)
- [ ] Database schema migrations are versioned and reversible (CWE-829)
...
- [ ] Encryption keys are managed via a dedicated key management service (CWE-320)
- [ ] Encryption key rotation is automated on a defined schedule (CWE-320)
- [ ] Backups are encrypted and access-controlled (CWE-311)
- [ ] Data classification labels are applied to distinguish sensitivity levels (CWE-200)
- [ ] Sensitive data is masked or redacted in non-production environments (CWE-200)
...
- [ ] Backup procedures run on schedule with verification (CWE-530)
...
- [ ] Third-party services have documented risk assessments (CWE-829)
- [ ] Vendor SOC 2 reports are reviewed annually (CWE-829)
- [ ] Data processing agreements are in place with all vendors handling customer data (CWE-829)
- [ ] Sub-processor lists are maintained and changes communicated (CWE-829)
...
- [ ] Security awareness training is documented for development teams (CWE-829)
